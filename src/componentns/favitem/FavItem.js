...
import { changeAddFavorites } from "../../store/slices/FavoritesSlice";
    ...
    return (
        <>
            ...
            <ul>
                ...
                <svg onClick = {()=>{
                    dispatch(addFavorites({id:id,index:index,products:products}))
                    dispatch(changeAddFavorites(index))
                dispatch(setProducts(JSON.parse(localStorage.getItem('products'))))}} version="1.0" xmlns="http://www.w3.org/2000/svg"
...
1388 -620 1525 -56 138 -104 253 -108 258 -3 4 -278 -610 -610 -1365z"/>
    ...
    dispatch(openModal())
     ...
     dispatch(SET_VALUE_INDEX(index))}}>Add cart</button> </ul>
      </>
    )
  }       
FavItem.propTypes  = {
